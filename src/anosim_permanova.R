...
calculate_anosim <-function(ps, folder_out){
  sample_data(ps)$location_type<-paste(sample_data(ps)$Location, sample_data(ps)$Type, sample_data(ps)$Stadium)
  locations_all<-unique(sample_data(ps)$location_type)
...
calculate_adonis <- function(ps, folder_out){
